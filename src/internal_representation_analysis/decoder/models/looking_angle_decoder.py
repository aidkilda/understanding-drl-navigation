...
from internal_representation_analysis.constants import ADAM_LEARNING_RATE_LOOKING_ANGLE
from internal_representation_analysis.constants import LAMBDA_L2_LOOKING_ANGLE
...
class LookingAngleDecoder(DecoderModel):
    def __init__(self,
                 scene_scope,
                 embedding,
                 x,
                 ...
                 input_size,
                 device="/cpu:0"):
        super(LookingAngleDecoder, self).__init__(
    ...
    def prediction(self):
        ...
        with tf.name_scope('W_fc2'):
            W_fc2 = fc_weight_variable([data_size, 1])
            self.W_fc2 = W_fc2

        with tf.name_scope('bias_fc2'):
            bias_fc2 = fc_bias_variable([1], data_size)

        with tf.name_scope('out_angle'):
            out_angle_ = tf.matmul(h_fc1_drop, W_fc2) + bias_fc2
    ...
    def optimize(self):
        ...
        optimizer_angle = tf.train.AdamOptimizer(learning_rate=ADAM_LEARNING_RATE_LOOKING_ANGLE)
    ...
    def accuracy(self):
        ...
        anticlockwise_err = tf.abs(pred_angle - self.theta)
        clockwise_err = 360 - anticlockwise_err
        error_angle = tf.minimum(anticlockwise_err, clockwise_err)
