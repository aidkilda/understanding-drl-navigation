...
class ActorCriticFFNetwork(ActorCriticNetwork):
  ...
  """
    ...
    def __init__(self,
        ...
        self.observation_embedding = dict()
        ...
        with tf.device(self._device):
            ...
            with tf.variable_scope(network_scope):
                ...
                h_s_flat = tf.nn.relu(
                    tf.matmul(self.s_flat, self.W_fc1[key]) + self.b_fc1[key])
                h_t_flat = tf.nn.relu(
                    tf.matmul(self.t_flat, self.W_fc1[key]) + self.b_fc1[key])
                h_fc1 = tf.concat(values=[h_s_flat, h_t_flat], axis=1)

                self.observation_embedding[key] = h_s_flat

                # shared fusion layer
                self.W_fc2[key] = self._fc_weight_variable([1024, 512])
                self.b_fc2[key] = self._fc_bias_variable([512], 1024)
                h_fc2 = tf.nn.relu(
                    tf.matmul(h_fc1, self.W_fc2[key]) + self.b_fc2[key])

                self.scene_specific_layer = dict()
                ...
                for scene_scope in scene_scopes:
                    ...
                    with tf.variable_scope(scene_scope):
                        ...
                        h_fc3 = tf.nn.relu(
                            tf.matmul(h_fc2, self.W_fc3[key]) + self.b_fc3[key]
                        )

                        self.scene_specific_layer[key] = h_fc3

                        # weight for policy output layer
                        self.W_policy[key] = self._fc_weight_variable(
                            [512, action_size])
                        self.b_policy[key] = self._fc_bias_variable(
                            [action_size], 512)

                        # policy (output)
                        pi_ = tf.matmul(
                            h_fc3, self.W_policy[key]) + self.b_policy[key]
                        ...
                        # value (output)
                        v_ = tf.matmul(h_fc3,
                                       self.W_value[key]) + self.b_value[key]
    ...
    def run_observation_embedding(self, sess, state, scopes):
        key_embed = self._get_key(scopes[:1])
        embed_out = sess.run(
            self.observation_embedding[key_embed], feed_dict={
                self.s: [state]
            })
        return embed_out[0]

    def run_scene_specific_layer(self, sess, state, target, scopes):
        k = self._get_key(scopes[:2])
        scene_specific_layer_out = sess.run(
            self.scene_specific_layer[k],
            feed_dict={
                self.s: [state],
                self.t: [target]
            })
        return scene_specific_layer_out[0]

    def get_vars(self):
        var_list = [
            self.W_fc1, self.b_fc1, self.W_fc2, self.b_fc2, self.W_fc3,
            self.b_fc3, self.W_policy, self.b_policy, self.W_value,
            self.b_value
        ]
        vs = []
        for v in var_list:
