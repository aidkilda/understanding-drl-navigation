...
from internal_representation_analysis.constants import ACTION_SIZE
from internal_representation_analysis.constants import SCREEN_WIDTH
from internal_representation_analysis.constants import SCREEN_HEIGHT
from internal_representation_analysis.constants import HISTORY_LENGTH
...
class THORDiscreteEnvironment(object):
    def __init__(self, config=dict()):
        ...
        self.n_feat_per_locaiton = config.get('n_feat_per_locaiton',
                                              1)  # 1 for no sampling
        self.terminal_state_id = config.get('terminal_state_id', 0)

        self.h5_file_path = config.get('h5_file_path',
                                       '/home/aidas/Masters/target-driven-visual-navigation-metric-map/code/data/%s.h5' % self.scene_name)
        ...
        self.current_shortest_distance = 0
    ...
    def reset(self):
        ...
        while True:
            ...
            self.current_shortest_distance = self.shortest_path_distances[k][
    ...
    def get_state(self, state_id):
        return self._tiled_state(state_id)
    ...
    def action_definitions(self):
        action_vocab = [
            "MoveForward", "RotateRight", "RotateLeft", "MoveBackward"
        ]
